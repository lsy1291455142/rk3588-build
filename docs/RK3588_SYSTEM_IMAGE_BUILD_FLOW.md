...
- 删除产物 DTB 中的 `/chosen/bootargs`，防止 Rockchip U-Boot 覆盖 extlinux 参数
- 安装模块并输出 `modules.tar`
...
Debian builder 固定为 `linux/arm64`，不会跟随宿主机生成 amd64 rootfs。
选择 `ROOTFS=debian` 后，`make build-rootfs` 会自动准备 builder：x86_64 Docker 主机注册
ARM64 binfmt/QEMU，ARM64 Docker 主机直接原生执行。
...
定位根文件系统，不依赖 initramfs 或固定的 `/dev/mmcblkXpY` 名称。打包使用的
DTB 不保留 `/chosen/bootargs`；串口和板级附加参数全部来自 profile 与 extlinux，
避免厂商 DTB 中的固定 `root=PARTUUID=...` 在 U-Boot 合并参数时覆盖该配置。

`idblock.img` 必须以 `RKNS` 开头。`download-loader.bin` 以 `LDR ` 开头，仅供
Rockchip USB 下载命令使用，不能直接写入 sector 64。
...
- GPT 主备表和两分区起止 LBA
- boot/rootfs 分区类型与名称
- RKNS IDBlock 和 U-Boot 固定偏移处的二进制内容
- FAT 中的 Image、DTB 和 extlinux
- 输出 DTB 与 FAT 内嵌 DTB 均不包含可覆盖 extlinux 的 `/chosen/bootargs`
...
`ROOTFS` 没有默认值。可通过 `make use-rootfs` 交互选择，或通过
`make use-rootfs-buildroot`、`make use-rootfs-debian`、`make use-rootfs-all` 写入
`.env`。未配置时，rootfs 构建、镜像打包和完整构建都会直接报错，不根据已有
输出目录猜测系统类型。
