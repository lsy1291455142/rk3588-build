...
`make build-rootfs ROOTFS=debian` 会自动准备 builder：x86_64 Docker 主机注册
...
当命令行和 `.env` 都未设置 `ROOTFS` 时，`make image` 与 `make verify-image` 会在
当前板型输出目录中查找已有 rootfs：只有一种时自动选择；Buildroot 和 Debian
同时存在时拒绝猜测，需显式指定 `ROOTFS=buildroot` 或 `ROOTFS=debian`。
