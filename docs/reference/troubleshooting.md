...
1. **用户态**：需要 WiFi 用户态时请在 `DEBIAN_PACKAGES` 中写上 `network-manager,wpasupplicant`。确认 `rootfs-build-info.txt` 里 `debian_packages` 含这些包名，且镜像里有 `/usr/sbin/wpa_supplicant`。
2. **固件 + 驱动**：内核模块在 kernel modules 包里；固件由板级 plugin 写入
   `rootfs/debian/boards/<BOARD>/overlay/lib/firmware/`（CokePi 含 `/vendor` 兼容链）。
   `make build-rootfs` 会从 `packages/*.deb` 自动装入 rootfs；host 可选手动 stage：
...
上板后 `dmesg` 看固件加载失败信息。换模组时更新对应板级 overlay 里的固件树即可，不必重编内核。
...
SDK 的 `external/rkwifibt/firmware` **没有** `aic8800D80/`。处理：板级 plugin
在 `make build-rootfs` 时从 `packages/*.deb` 装入 rootfs（默认 3.0）。host 可选手动：

```bash
./rootfs/debian/boards/rk3588s-cokepi-model-lp4-v10/stage-aic8800-firmware.sh
ls rootfs/debian/boards/rk3588s-cokepi-model-lp4-v10/overlay/lib/firmware/aic8800D80/
make build-rootfs
```
...
正常。本 builder 不假设完整 BSP `external/`。WiFi/BT 由**板级静态 overlay** 提供（host 侧可从 `.deb` stage）。不要为了通用 builder 去硬依赖某一家的 `external` 树。
