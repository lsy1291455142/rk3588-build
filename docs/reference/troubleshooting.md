...
2. **固件 + 驱动**：内核模块（如 `bcmdhd`）在 kernel modules 包里；固件必须用 `WIFIBT_CHIP`（wifibt 插件）装进 `/lib/firmware`（并有 `/vendor` 兼容链）。

```bash
# 有厂商 deb 就装 deb；否则再考虑 BSP 静态树
./rootfs/debian/overlays/wifibt/sync-assets.sh --deb /path/to/firmware.deb
# 或：./rootfs/debian/overlays/wifibt/sync-assets.sh --from-bsp /path/to/full-bsp AP6275S
...
```bash
# 推荐：Radxa aic8800-firmware deb
./rootfs/debian/overlays/wifibt/sync-assets.sh --deb-aic
ls rootfs/debian/overlays/wifibt/packages/aic8800-firmware_*.deb
...
正常。本 builder 不假设完整 BSP `external/`。WiFi/BT 由可选 `wifibt` overlay 以**固件包/静态文件**方式提供（优先 `.deb`）。不要为了通用 builder 去硬依赖某一家的 `external` 树。
