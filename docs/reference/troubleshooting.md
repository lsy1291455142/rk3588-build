...
```bash
# 从完整 BSP 同步固件到 wifibt overlay（最小 SDK 无 external/rkwifibt 时）
...
```bash
# 确认 overlay 本地树里有固件（或遗留 assets/wifibt）
ls rootfs/debian/overlays/wifibt/firmware/aicsemi/AIC8800D80/fw_patch_table_8800d80_u02.bin
...
正常。本 builder 不假设完整 BSP `external/`。WiFi/BT 固件由可选 `wifibt` overlay 提供（`overlays/wifibt/firmware/`、遗留 `assets/wifibt`，或你自己 import 的完整 SDK）。不要为了通用 builder 去硬依赖某一家的 `external` 树。
